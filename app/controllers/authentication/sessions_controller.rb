class Authentication::SessionsController < ApplicationController
  ...
  def create
    ...
    user = User.find_by(
      "lower(username) = :login OR lower(email) = :login",
      login: login_input
    )

    if user&.authenticate(params[:password])
      session[:user_id] = user.id
      redirect_to products_path, notice: t("authentication.sessions.notices.welcome", username: user.username)
    else
  ...
  def destroy
    ...
    redirect_to products_path, notice: t("authentication.sessions.notices.destroyed")
  end
end
